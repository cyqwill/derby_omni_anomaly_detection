...
def load_data(dataset):
    if dataset == 'SMD':
        ...
        os.makedirs(os.path.dirname(label_folder), exist_ok=True)
    ...
    elif dataset == 'derbysoft':
        dataset_folder = 'DerbysoftServerMachineDataset'
        file_list = os.listdir(os.path.join(dataset_folder, "train"))
        for filename in file_list:
            if filename.endswith('.txt'):
                load_and_save('train', filename, filename.strip('.txt'), dataset_folder)
            ...
            if filename.endswith('.csv'):
                load_and_save('train', filename, filename.strip('.csv'), dataset_folder)
                load_and_save('test', filename, filename.strip('.csv'), dataset_folder)
                load_and_save('test_label', filename, filename.strip('.csv'), dataset_folder)


if __name__ == '__main__':
    datasets = ['SMD', 'SMAP', 'MSL','derbysoft']
    # commands = sys.argv[1:]
    commands = ['derbysoft']
    load = []
    if len(commands) > 0:
